...
class AlphabetaPlayer:
    ...
    def dynamic_weights(self, bitboard):
        ...
        if count < 6:
            return {"group": 1.0, "influence": 1.2, "territory": 0.5, "liberty": 1.5, "ko": 1.0}
        elif count < 15:
            return {"group": 1.0, "influence": 0.7, "territory": 0.7, "liberty": 1.0, "ko": 1.0}
        else:
            return {"group": 1.2, "influence": 0.5, "territory": 1.0, "liberty": 0.8, "ko": 1.0}
    ...
    def pattern_bonus(self, bitboard, move, side):
        ...
        new_board = self.simulate_move(bitboard, i, j, side)
        opponent = STONE_BLACK if side == STONE_WHITE else STONE_WHITE
        before = sum(1 for x in range(BOARD_SIZE) for y in range(BOARD_SIZE) if get_cell(bitboard, x, y) == opponent)
        after = sum(1 for x in range(BOARD_SIZE) for y in range(BOARD_SIZE) if get_cell(new_board, x, y) == opponent)
        ...
        return capture_bonus + connection_bonus
    ...
    def alphabeta(self, bitboard, depth, alpha, beta, maximizingPlayer):
        ...
        if maximizingPlayer:
            ...
            for move in legal_moves:
                ...
                if alpha >= beta:
                    # Beta cutoff: 记录 killer move 和更新历史启发
